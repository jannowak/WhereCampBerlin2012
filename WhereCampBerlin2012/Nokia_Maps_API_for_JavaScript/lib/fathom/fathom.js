	...
	Fathom.prototype = {
		...
		_setMargins: function() {
			var displayMode = this.config.displayMode,
				...
				verticalSpacing = Math.ceil(($container.height() - this.$firstSlide.innerHeight() - this.config.topOffset) / 2),
			...
			this.$container.css('margin-top', verticalSpacing + this.config.topOffset);
